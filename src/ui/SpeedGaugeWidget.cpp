...
void SpeedGaugeWidget::paintEvent(QPaintEvent*) {
    ...
    // Needle - with margin to stay inside the arc visually
    const double margin = 8.0; // degrees of padding on each side
    const double norm = (m_value - m_min) / (m_max - m_min);
    const double needleDeg = (startDeg - margin) - (spanDeg - 2*margin) * norm;
    ...
    // Needle cap
    // ✅ Center in black (contour + filling)
    p.setBrush(Qt::black);
    p.setPen(Qt::NoPen);
    p.drawEllipse(c, 8, 8);

    // Center value text (already in black)
    p.setPen(Qt::black);
    QFont f = font();
    f.setBold(true);
    f.setPointSize(18);
