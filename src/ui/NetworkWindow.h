...
private:
    ...
    // UI
    QLineEdit* m_backendUrl{};       // Server 1 (local)
    QLineEdit* m_backendUrl2{};      // Server 2 (local)
    QLineEdit* m_backendUrlReal{};   // Real Server (AlwaysData)
    QPushButton* m_switchBtn{};
    int          m_activeServer{1};  // 1, 2 or 3
    QLabel*      m_activeServerLbl{};
    QLabel*      m_proofLbl{};
    ...
    // Auto refresh
    QTimer* m_pingTimer{};
    bool m_pingInFlight{false}; // prevents stacking requests
    int  m_failCount{0};        // consecutive failures for auto-switch
